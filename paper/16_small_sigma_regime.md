...
results are floating-point observations with certified prime-truncation
budgets; the structural remarks are unproved orientation, labeled as such.
...
doubling for which the certified eigenvalue-shift bound of
`paper/14_certified_prime_tail.md` is below \(10^{-8}\).  The zero-side
column rebuilds the matrix from actual zeta zeros (computed at run time when
`mpmath` is available) down to envelope \(10^{-15}\).
...
1. **Positivity within budget at every width.**  Each \(\lambda_{\min}\) lies
   inside the certified truncation budget of zero.  Nothing here proves
   positivity of any infinite object; the point is that the finite model
   behaves exactly as the validated zero-side identity dictates.
...
3. **The certified cutoff collapses at small widths.**  At \(\sigma=0.12\)
   the certified budget is met with cutoff \(84\): narrow kernels localize
   the prime sum near \(\log n\approx|\delta_{ij}|\le3\), so very few prime
   powers matter.  The expensive regime is *wide* packets, not narrow ones.
...
positive-definiteness of the transform of a nonnegative measure.  Under GRH
the measure is supported on the real line and positivity is automatic; a
hypothetical off-line zero would make \(K\) the transform of a signed or
complex-shifted configuration, and its positive-definiteness could fail in
sufficiently resolved finite sections.  This gives the black-swan search of
`paper/03_spectral_program.md`, section 10, a concrete home: it is a search
for a Toeplitz section whose symbol dips negative.
...
\(\sigma\) (opening the zero window, with cheap certified cutoffs) rather
...
2. The certified budget currently uses a floating-point
   \(\lambda_{\min}(B)\); replace it with a directed-rounding enclosure to
   make rows of the scaling table fully certified.
