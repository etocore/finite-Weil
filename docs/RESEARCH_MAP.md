...
| Identification of `A_prime` with the classical explicit-formula prime term | Observed at machine precision (`paper/13_pole_cancellation.md`; committed script and artifact for five characters) | Entrywise agreement of the full assembly with independently computed zero-side matrices | Upgrade the floating-point comparison to an interval certificate |
...
| Deep-cutoff negativity for `D = 1` is the missing pole block | Observed (`paper/13_pole_cancellation.md`) | With-pole spectrum collapses to `~1e-5` at cutoff `1e7` | Analytic truncation budget available via `paper/14_certified_prime_tail.md` (floating-point evaluated) |
| Entrywise prime-tail bound and eigenvalue-shift budget on fixed packet spaces | Proved (`paper/14_certified_prime_tail.md`); evaluated in floating point | Closed-form theorem, quadrature and refinement-domination tests | Directed-rounding evaluation of the whole bound chain for full certification |
| Low-lying zeros are identifiable from finite prime data | Observed (`paper/15_zero_recovery.md`, artifact `paper/data/zero-recovery.csv`) | Complex-mode pencil recovery of the first 14 zeta zeros with envelope-consistent amplitudes and unit-circle decay rates | Quantify off-line detection sensitivity; extend to quadratic characters |
| Small-bandwidth positivity within analytic truncation budget | Observed (`paper/16_small_sigma_regime.md`) | Sigma sweep with bound-resolved cutoffs and zero-side comparison | Szego-type limit of the whitened Toeplitz spectra |
